    ...
    namespace Neow {
        ...
        CardReward getColorlessCardReward(Random &neowRng, Random& cardRng, bool rareOnly) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                if (rareOnly) rarity = CardRarity::RARE;
                else if (rarity == CardRarity::COMMON) rarity = CardRarity::UNCOMMON;

                Card card = Card(Game::getRandomColorlessCardNeow(cardRng, rarity));
                while (true) {
                    ...
                    if (containsCard) card = Card(Game::getRandomColorlessCardNeow(cardRng, rarity));
                    else break;
                }
